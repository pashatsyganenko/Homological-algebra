...
#proof[
  Из доказательства теоремы выше видно, что существует главный неразложимый модуль $P ->>^f S$. Ядро накрытия $f$ является малым модулем и следовательно содержится в $Rad(P) = J(A)P$. Однако так как модуль $S$ является полупростым, то #box($0 = Rad(S) = Rad((P slash ker(f))) = Rad(P) slash ker(f)$), из чего имеем $ker(f)= Rad(P) = J(A)P$.В
]
