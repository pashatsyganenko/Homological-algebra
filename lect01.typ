...
То есть $forall N lt.eq.slant M : thick exists P lt.eq.slant M : thick M = P plus.circle N$.
...
#let semisimple-note-label = <semisimple-note>;
...
То есть полупростое артиново кольцо разлагается в прямое произведение матричных колец над телами, причём конечное произведение матричных колец  -- полупросто.
...
#proof[
  Допустим существует $x in Rad(M) without {0}$. Как и всякий подмодуль, $A x$ можно выделить в прямую сумму: $M = Ax plus.circle U$. Если рассмотреть стандартный эпиморфизм $A -> A x : med 1 arrow.bar x$, то станет очевидно, что $A x tilde.equiv A slash #h(-0.1mm) Ann(x)$. Вложение $Ann(x)$ в максимальный идеал $I$, его содержащий, индуцирует эпиморфизм $A slash #h(-0.1mm) Ann(x) arrow.twohead A slash I$, причём $S := A slash I$ -- простая алгебра. Имеем композицию
...
#proof[
  ...
  #align(center)[
  #scam-alert[
    В этом месте Семенов решил заскамить аудиторию! Ведуться работы по устранению ошибок, сохраняйте спокойствие!
  ]
  ]
]
