...
Рассмотрим короткую точную последовательность $A >->^alpha B ->>^beta C$, назовём её $E$, и стрелку $f: C' -> C$. Построим пулбэк $f$ и $beta$, пусть это $B_1$ со стрелками $beta_1 : B_1 -> C'$ и $h: B_1 -> B$. Тогда $beta_1$ #link(<universal-square-lemma>)[тоже эпиморфизм]. Заведём дополнительно нулевую стрелку из $A$ в $C$. Тогда взяв предел получим единственное $alpha_1 : A -> B_1$ такое, что $beta_1 alpha_1 = 0$ и $h alpha_1 = alpha$. Заметим, что подходит $alpha_1 = ker(beta_1)$ #comment[в каком плане?]
