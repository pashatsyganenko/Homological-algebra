...
#corollary[
  ...
  #comment[
  Вообще это верно и в некоммутативном случае.
  ]
]

Ниже под $R$ понимается ассоциативное кольцо с единицей.
#definition[
  Подмодуль $X$ модуля $bl(M,R)$ называется малым, если для всякого $K lt.eq.slant M$\ из $X+K=M$ следует $K = M$.
]

#lemma[
  Пусть $A$ -- конечномерная алгебра, $bl(M,R)$ -- левый $A$-модуль. Тогда
...
#proof[
  ...
  $ M arrow.twohead^(pi_1) M/K = (A x)/(A x inter K) =: bl(D,A) tilde.equiv^(pi_2) A/Ker((A arrow.long_(1 arrow.bar x) M)) arrow.twohead^(pi_3) A/I = S. $
  Первое равенство получено по второй теореме об изоморфизме. $bl(D,A)$ -- циклический модуль. $I$ -- некоторый максимальный подмодуль, в который вложено ядро. $S$ -- простой модуль.\
...
#theorem[
  Если $A$ -- конечномерная алгебра и $bl(M,A)$ -- модуль, то $Rad(M)$ мал.
]
#proof[
...
#definition[
  Модуль $bl(P,R)$ называется проективным, если $ forall f : P -> N quad forall sigma : M arrow.twohead N quad exists g: P -> M : f = sigma g. $
...
#proof[\
  ...
  $arrow.double.l$: Очевидно следует из лемм #numbered-link(<free-proj>)[] и #numbered-link(<sum-proj>)[].
]

== Проективные накрытия
Пусть $A$ -- конечномерная алгебра над полем $K$.
#definition[
  Можно представить $bl(A,A)$ как $bl(A,A) = plus.circle.big_(j=1)^n P_j$, где все $P_j$ неприводимые. Они называются главными неразложимыми модулями.
]
...
#definition[
  $(bl(P,A), pi: P ->> bl(M,A))$ -- проективное накрытие модуля $M$ #iff $Ker(pi)$ мало.
]
...
#theorem[
  $bl(S,A)$ -- просто $med arrow.double.long med$ $exists (P, pi : P ->> S)$ -- проективное накрытие. // костыльны, конечно, эти med
]
#proof[
