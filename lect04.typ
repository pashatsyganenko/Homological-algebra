...
#corollary[
  Если $L$ инъективен, $i: N -> M$, $j: N -> L$ мономорфизмы и $Im(i)$ существеннен в $M$, то гомоморфизм $g: M -> L$ такой, что $j = g i$ (он существует так как $L$ инъективен) мономорфен.
]
#proof[
  Дословно повторяем действия из доказательства теоремы. Если $Ker(g) eq.not 0$, то $Im(i) inter Ker(j) eq.not 0$.
  Выберем ненулевой $x in Im(i) inter Ker(g)$, тогда $x = i(y)$ и $j(y) = g i (y) = g(x) = 0$,  значит $x = y = 0$. Противоречие.
]

#corollary[
  $bl(Q,A)$ инъективен $arrow.double$ $Q$ есть прямая сумма неразложимых конечнопорождённых.
]
#proof[
  (Q >-> Q, Q) -- инъективная оболочка $Q$. Понятно, что как и проективное накрытие, инъективная оболочка единственна, так что $Q$ совпадает с $plus.circle.big_(i in I) Q_i$ из только что доказанной теоремы. $Q_i$ строились как инъективные оболочки простых модулей, которые по #numbered-link(<simple-inj-hull>)[теореме] конечнопорождены.
]
